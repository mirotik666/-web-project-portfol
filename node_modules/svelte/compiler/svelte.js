...
function visitBinding(generator, block, state, node, attribute, local) {
    ...
    var observer = block.getUniqueName('observer');
    var value = block.getUniqueName('value');
    local.create.addBlock((_b = ["\n\t\tfunction ", " ( value ) {\n\t\t\tif ( ", " ) return;\n\t\t\t", " = true;\n\t\t\t", "\n\t\t\t", " = false;\n\t\t}\n\n\t\t", ".observe( '", "', ", ", { init: false });\n\n\t\t#component._root._beforecreate.push( function () {\n\t\t\tvar value = ", ".get( '", "' );\n\t\t\tif ( @differs( value, ", " ) ) {\n\t\t\t\t", ".call( ", ", value );\n\t\t\t}\n\t\t});\n\t"], _b.raw = ["\n\t\tfunction ", " ( value ) {\n\t\t\tif ( ", " ) return;\n\t\t\t", " = true;\n\t\t\t", "\n\t\t\t", " = false;\n\t\t}\n\n\t\t", ".observe( '", "', ", ", { init: false });\n\n\t\t#component._root._beforecreate.push( function () {\n\t\t\tvar value = ", ".get( '", "' );\n\t\t\tif ( @differs( value, ", " ) ) {\n\t\t\t\t", ".call( ", ", value );\n\t\t\t}\n\t\t});\n\t"], deindent(_b, observer, updating, updating, setter, updating, local.name, attribute.name, observer, local.name, attribute.name, snippet, observer, local.name)));
...
var preprocessors = {
    ...
    IfBlock: function (generator, block, state, node, inEachBlock, elementStack, stripWhitespace, nextSibling) {
        var blocks = [];
        var dynamic = false;
        var hasIntros = false;
        var hasOutros = false;
        function attachBlocks(node) {
            ...
            if (isElseIf$1(node["else"])) {
                ...
                preprocessChildren(generator, node["else"]._block, node["else"]._state, node["else"], inEachBlock, elementStack, stripWhitespace, nextSibling);
    ...
    EachBlock: function (generator, block, state, node, inEachBlock, elementStack, stripWhitespace, nextSibling) {
        var dependencies = block.findDependencies(node.expression);
        block.addDependencies(dependencies);
    ...
    Element: function (generator, block, state, node, inEachBlock, elementStack, stripWhitespace, nextSibling) {
        node.attributes.forEach(function (attribute) {
...
function preprocessChildren(generator, block, state, node, inEachBlock, elementStack, stripWhitespace, nextSibling) {
    ...
    cleaned.forEach(function (child, i) {
        ...
        if (preprocessor)
            preprocessor(generator, block, state, child, inEachBlock, elementStack, stripWhitespace, cleaned[i + 1] || nextSibling);
        if (lastChild) {
...
var Selector$2 = (function () {
    ...
    Selector.prototype.transform = function (code, attr, id) {
...
var Atrule$2 = (function () {
        ...
        if (this.node.block) {
            ...
            this.rules.forEach(function (rule) {
                if (cascade || rule.isUsed()) {
                    code.remove(c_2, rule.node.start);
    ...
    Atrule.prototype.transform = function (code, id, keyframes) {
        if (this.node.name !== 'keyframes')
            return;
        this.node.expression.children.forEach(function (expression) {
            if (expression.type === 'Identifier') {
                if (expression.name.startsWith('-global-')) {
                    code.remove(expression.start, expression.start + 8);
                }
                else {
                    var newName = id + "-" + expression.name;
...
var Stylesheet = (function () {
    function Stylesheet(source, parsed, filename, cascade) {
        ...
        if (parsed.css && parsed.css.children.length) {
            ...
            walk(this.parsed.css, {
                enter: function (node) {
                    if (node.type === 'Atrule') {
                        var atrule = currentAtrule_1 = new Atrule$2(node);
                        stack_1.push(atrule);
                        _this.nodes.push(atrule);
                        _this.atrules.push(atrule);
                    }
                    if (node.type === 'Rule') {
    ...
    Stylesheet.prototype.render = function (cssOutputFilename) {
        ...
        this.atrules.forEach(function (atrule) {
            atrule.transform(code, _this.id, keyframes);
        });
        this.rules.forEach(function (rule) {
            rule.transform(code, _this.id, keyframes, _this.cascade);
        });
        var c = 0;
        this.nodes.forEach(function (node) {
            if (_this.cascade || node.isUsed()) {
                code.remove(c, node.node.start);
                node.minify(code, _this.cascade);
                c = node.node.end;
            }
        });
